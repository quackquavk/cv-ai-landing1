...
> **v2.0 Dark Mode:** Each table below carries paired Light Hex / Dark Hex values. The Dark Hex column is the authoritative source for `light-dark()` second arguments in `app/globals.css`. All dark values derived per D-01 (Stripe-informed dark-mode conventions); contrast ratios verified against `#0d1f35` dark surface.
...
|-------|------|-----------|---------|
| `--ds-raw-purple-500` | Primary brand / CTA | `#533afd` | `#8679fe` |
| `--ds-raw-purple-600` | Hover state | `#4434d4` | `#8c81fe` |
| `--ds-raw-purple-700` | Icon hover / deep | `#2e2b8c` | `#9d94fe` |
| `--ds-raw-purple-light` | Subdued hover bg | `#b9b9f9` | `#3d3571` |
| `--ds-raw-purple-mid` | Range selector | `#665efd` | `#8880fe` |
...
| Token | Role | Light Hex | Dark Hex |
|-------|------|-----------|---------|
| `--ds-raw-brand-dark` | Brand dark sections / fixed | `#1c1e54` | `#1c1e54` |
| `--ds-raw-navy-975` | Deep Navy heading text | `#061b31` | `#e8f0f8` |
| `--ds-raw-navy-900` | Dark neutral | `#0d253d` | `#c8d8e8` |
...
| Token | Role | Light Hex | Dark Hex |
|-------|------|-----------|---------|
| `--ds-raw-ruby` | Ruby accent | `#ea2261` | `#f15080` |
| `--ds-raw-ruby-text` | Error text (passes 4.5:1 both schemes) | `#c41a4f` | `#f15080` |
| `--ds-raw-magenta` | Magenta accent | `#f96bee` | `#fb8ef4` |
| `--ds-raw-magenta-light` | Magenta tint surface | `#ffd7ef` | `#3a1e35` |
...
- **Success Text** (`#0d7a35`): Success badge text color. (Retuned in v2.0 A11Y-02: 5.45:1 on white.)
- **Lemon** (`#9b6829`): `--hds-color-core-lemon-500`. Warning and highlight accent.

| Token | Role | Light Hex | Dark Hex |
|-------|------|-----------|---------|
| `--ds-raw-slate-label` | Label text | `#273951` | `#a8bdd4` |
| `--ds-raw-slate-body` | Body text | `#64748d` | `#7596b5` |
| `--ds-raw-green-500` | Success green | `#15be53` | `#22d46a` |
| `--ds-raw-green-text` | Success badge text | `#0d7a35` | `#34c96e` |
| `--ds-raw-lemon` | Warning / highlight | `#9b6829` | `#d4a54a` |
| `--ds-raw-warning-text` | Warning badge text (passes 4.5:1 both schemes) | `#7a4f1c` | `#d4a54a` |
| `--ds-raw-info-text` | Info badge text (passes 4.5:1 both schemes) | `#1a5e94` | `#4aa3db` |
...
| Token | Role | Light Hex | Dark Hex |
|-------|------|-----------|---------|
| `--ds-raw-white` | App surface | `#ffffff` | `#0d1f35` |
| `--ds-raw-neutral-pill` | Raised surface / pill bg | `#f6f9fc` | `#132d4a` |
| `--ds-raw-border` | Default border | `#e5edf5` | `#1e3048` |
| `--ds-raw-border-purple` | Active / selected border | `#b9b9f9` | `#4d45a8` |
| `--ds-raw-border-soft` | Soft border | `#d6d9fc` | `#2d3070` |
| `--ds-raw-border-dashed` | Dashed / drop zone border | `#362baa` | `#4a42b8` |

Note on border/surface tokens: contrast ratios below 4.5:1 for border/surface tokens are expected — these are structural separators and tint backgrounds, not text colors. WCAG AA 3:1 applies to non-text UI elements.

### Info

| Token | Role | Light Hex | Dark Hex |
|-------|------|-----------|---------|
| `--ds-raw-info-blue` | Info blue | `#2874ad` | `#4aa3db` |
...
### 6.5 Dark Shadow System

On dark surfaces (`#0d1f35` deep navy), the v1.0 blue-tinted shadows (`rgba(50,50,93,0.25)`) are nearly invisible — the chromatic difference against deep navy is too small. Dark-mode shadows use pure black at elevated opacity (0.40–0.75 range) to register elevation against dark backgrounds. The elevation hierarchy is preserved: higher levels use proportionally higher opacity.

| Level | Token | Light Shadow Value | Dark Shadow Value |
|-------|-------|-------------------|------------------|
| Flat (0) | `--ds-shadow-flat` | `none` | `none` |
| Ambient (1) | `--ds-shadow-ambient` | `rgba(23, 23, 23, 0.06) 0px 3px 6px` | `rgba(0, 0, 0, 0.40) 0px 3px 6px` |
| Standard (2) | `--ds-shadow-standard` | `rgba(23, 23, 23, 0.08) 0px 15px 35px 0px` | `rgba(0, 0, 0, 0.50) 0px 15px 35px 0px` |
| Elevated (3) | `--ds-shadow-elevated` | `rgba(50, 50, 93, 0.25) 0px 30px 45px -30px, rgba(0, 0, 0, 0.10) 0px 18px 36px -18px` | `rgba(0, 0, 0, 0.65) 0px 30px 45px -30px, rgba(0, 0, 0, 0.40) 0px 18px 36px -18px` |
| Deep (4) | `--ds-shadow-deep` | `rgba(3, 3, 39, 0.25) 0px 14px 21px -14px, rgba(0, 0, 0, 0.10) 0px 8px 17px -8px` | `rgba(0, 0, 0, 0.75) 0px 14px 21px -14px, rgba(0, 0, 0, 0.45) 0px 8px 17px -8px` |
| Top-sticky | `--ds-shadow-top-sticky` | `rgba(0, 55, 112, 0.08) 0px -4px 8px 0px` | `rgba(0, 0, 0, 0.50) 0px -4px 8px 0px` |

**Opacity rationale:**
| Level | Light Opacity | Dark Opacity | Boost Factor |
|-------|-------------|-------------|--------------|
| Ambient | 0.06 | 0.40 | 7x — minimal lift needs big boost for dark visibility |
| Standard | 0.08 | 0.50 | 6x — card shadow |
| Elevated primary | 0.25 | 0.65 | 2.6x — blue-tinted → black |
| Elevated secondary | 0.10 | 0.40 | 4x — near-layer reinforcement |
| Deep primary | 0.25 | 0.75 | 3x — modal shadow; at opacity ceiling |
| Deep secondary | 0.10 | 0.45 | 4.5x — near-layer reinforcement |
| Top-sticky | 0.08 | 0.50 | 6x — directional upward shadow |

All 6 shadow tokens are wrapped in CSS `light-dark(<light-value>, <dark-value>)` in `app/globals.css`. The entire multi-layer shadow value is one argument to `light-dark()` — the function returns the complete `box-shadow` property value for the active scheme.
...
### Dark Mode — Do
- Use `light-dark(<light-hex>, <dark-hex>)` on `--ds-raw-*` tokens for scheme-adaptive Tier A color resolution
- Use paired `:root` + `[data-theme='dark']` blocks for the shadcn HSL shim (Tier B) — never `light-dark()` on HSL triplet vars
- Use `color-mix(in srgb, var(--ds-color-primary) 8%, var(--ds-color-surface))` for alpha tints — the surface token already resolves per scheme
- Keep `#resume-preview { color-scheme: light; }` as the sole `color-scheme` carve-out in the entire codebase

### Dark Mode — Don't
- Don't use `dark:*` Tailwind utilities — `light-dark()` in the token layer makes per-utility overrides redundant and creates two sources of truth
- Don't set `color-scheme` on individual surfaces or components — only `:root { color-scheme: light dark }` and the `#resume-preview` scope guard are permitted
- Don't author tints with hardcoded `rgba(255, 255, 255, 0.08)` — the alpha baseline flips wrong on dark surfaces; use `color-mix()` against `var(--ds-color-surface)` instead
- Don't wrap `--background`, `--foreground`, `--primary`, `--border`, `--card`, `--popover`, `--secondary`, `--muted`, `--accent`, `--destructive`, `--input`, `--ring`, or `--sidebar-*` in `light-dark()` — these are HSL triplets consumed by `hsl(var(...))` in Tailwind; wrapping them produces invalid CSS and silently kills every shadcn primitive
